  ...
  <div id="ProductDetail">
    ...
    <div id="show_product_detail" class="container-fluid">
      ...
      <div class="row m-auto">
        <div class="col-12 col-md-6">
          <div class="container-fluid">
            ...
            <div class="row w-100 m-0 mt-md-2">
              <div class="small-pic" v-for='(img,index) in showpic' :key='index'>
        ...
        <div class="col-12 col-md-6 description">
          ...
          <h2 class="mt-2 mx-3 pb-2 pb-md-3 border-bottom price">{{product.price | thousands}}</h2>
          <div class="description-content mx-3">
            <h4 class="d-block">【產品描述】</h4>
            <p class="p-2">{{ product.description }}</p>
          </div>
          <div class="control">
            <div class="countNumber d-flex justify-content-start align-items-center mx-3">
              <button class="btn minus" @click="count(-1)">
                <i class="fas fa-minus"></i>
              </button>
              <input class="productNumber" type="number" max='10' min='1' v-model="quantity">
              <button class="btn add" @click="count(1)">
                <i class="fas fa-plus"></i>
              </button>
            </div>
            <button class="btn productbuy mt-4 mt-md-5 py-md-3 w-md-100 mx-3" @click="addProduct(product)">
              加入購物車
    ...
    <div id="maybe_you_like" class="container-fliud p-0">
      ...
      <div class="row p-5 py-md-3 m-auto">
        <div class="like_item col-12 col-md-3 m-0 m-lg-5" v-for="item in sameitem" :key="item._id" @click='goDetail(item._id)'>
          ...
          <b-tabs content-class="mt-3" justified>
            <b-tab class="p-3" title=" 商品須知" active>
              <ul>
                ...
                <li>商品情境照為示意用，僅商品主體不包含其他配件，請以規格內容物為主。</li>
              </ul>
            </b-tab>
            ...
            <b-tab class="p-3" title="付款方式">
              <p>信用卡安全加密付款, 信用卡安全加密付款, 7-11 ibon 代碼繳費, ATM 轉帳繳費, 全家 FamiPort 代碼繳費, 信用卡分期 (3 期零利率), 信用卡分期 (6 期零利率), LINE Pay, Alipay 支付寶</p>
            </b-tab>
            <b-tab class="p-3" title="退貨須知">
              <li>依《消費者保護法》規定，消費者享有商品簽收翌日起算七天之鑑賞期，期間申請退購無須負擔運費，欲退購者請於七日內提出，逾期恕不受理。注意！猶豫期並非試用期，故退回的商品必須是全新的狀態且完整包裝（含商品本體、配件、贈品、保證書、原廠包裝及所有附隨文件或資料），切勿缺漏任何配件、自行拆解商品、或損毀原廠外盒。如有遺失、毀損或缺件，可能影響您退貨的權益，也將依照損毀程度扣除為復原所需之整新費用。</li>
              <li>如購買的商品是電腦軟體、遊戲、影音光碟、食品、耗材、個人衛生用品及客製化商品，拆封後除瑕疵品外恕無法辦理退換貨。</li>
            </b-tab>
          </b-tabs>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    getProduct() {
      this.axios.get(process.env.VUE_APP_API + '/products/product/' + this.$route.params.productId)
      .then((res)=>{
        this.product = res.data.result
        this.selectPic = process.env.VUE_APP_API + '/products/images/' + this.product.images[0].file
        for(let i = 0 ; i < this.product.images.length;i++){
          this.showpic.push(process.env.VUE_APP_API + '/products/images/' + this.product.images[i].file)
        }
    ...
    getProducts() {
      ...
      .then((res)=>{
        this.sameproducts = res.data.result.filter((item)=>{
          return item.type === this.product.type && item._id !== this.product._id
        })
        if (this.sameproducts > 3) {
          for (let i = 0; i < 3; i++) {
            const r = Math.floor(Math.random() * this.sameproducts.length)
            const idx = this.sameitem.findIndex(item => item._id === this.sameproducts[r]._id)
            console.log(idx);
            if (idx !== -1) {
              i--
            } else {
        ...
        this.sameitem.map((product)=>{
          product.src = process.env.VUE_APP_API + '/products/images/' + product.images[0].file
        })
      })
    },
    goDetail(id) {
    ...
    addProduct(product) {
      const data = {
          ...
          src: process.env.VUE_APP_API + '/products/images/' + product.images[0].file,
          price: product.price,
