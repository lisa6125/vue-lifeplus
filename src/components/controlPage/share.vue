  ...
  <div id="Share" class="container-fluid p-5">
    <div class="row mx-5 px-5 pb-4 border-bottom border-dark">
      ...
      <div class="col-5 text-right">
        ...
        <div class="btn btn-outline-secondary mr-3" @click="filter='等待審核'">未審核</div>
        <div class="btn btn-outline-secondary mr-3" @click="filter='審核通過'">審核通過</div>
        <div class="btn btn-outline-secondary mr-3" @click="filter='審核不通過'">審核不通過</div>
      </div>
    </div>
    <div class="share-view">
        <div v-masonry="containerId" transition-duration="0.3s" item-selector=".item">
          <div v-masonry-tile class="item" v-for="(item, index) in sharefilter" :key='index' >
            <div class="post border">
              ...
              <div class="status my-3" v-else>
                目前為:{{item.onshare}}
              </div>
              <div class="edit_del" >
                <div class="btn btn-outline-warning mr-2" @click="cancel(item)" v-if="item.edit">取消</div>
                <div class="btn btn-outline-success mr-2" @click="item.edit = true" v-else>編輯</div>
                <div class="btn btn-outline-success mr-2" @click="save(item,idx)" v-if="item.edit===true">保存</div>
                <div class="btn btn-outline-danger" @click="del(item,idx)">刪除</div>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    save(item,idx){
      ...
      this.axios.patch(process.env.VUE_APP_API + '/shares/'+ item._id ,{
